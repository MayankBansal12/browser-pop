...
let currentStatusBtn = document.getElementById("current-status-btn")
...
currentStatusBtn.addEventListener("click", () => {
  chrome.storage.sync.get({ isActive: false }, (data) => {
...
async function refreshExtensionStatus() {
  const data = await chrome.storage.sync.get({ isActive: false })
  const isActive = data?.isActive
